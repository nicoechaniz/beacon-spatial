...
This module is intentionally simple and self-contained. It uses librosa + numpy
(and optionally sounddevice for local playback). Migrated from digital-beacon
(T3.1); OSC wiring is deferred to T3.2.

A sample is read into memory, played in a loop, and analyzed in chunks. The
resulting descriptors (energy, f0 ratio, spectral centroid, etc.) can later be
published as OSC messages that drive the beacon.
"""
...
class SampleLayer:
    ...
    def _load(self) -> None:
        ...
        log.info(
            "SampleLayer loaded %s: sr=%d length=%.2fs frames=%d",
            self.path.name,
            sr_loaded,
            len(self._y) / sr_loaded,
            len(self._y),
        )
        ...
        try:
            ...
            flat = 0.0
        try:
            sep = self._resonant_filter.separate(
                self._y,
                self.f0_beacon_hz,
                flatness=flat,
                inharmonicity=0.0,
    ...
    def _analyze(
        self,
        chunk: np.ndarray,
        h_chunk: np.ndarray = np.zeros(0),
        r_chunk: np.ndarray = np.zeros(0),
    ) -> SampleDescriptor:
    ...
    def start(self) -> None:
        ...
        self._thread.start()
        log.info(
            "SampleLayer started: chunk=%d samples @ %.0f Hz, bands=%d",
            self.chunk_size,
            1.0 / self.chunk_s,
            self.N_BANDS,
        )
